...
export class GameSocket {
  ...
  private setupEventHandlers(): void {
    this.io.on('connection', (socket: Socket) => {
      ...
      // 加入房间事件
      socket.on('join_room', (data: { playerId: string; playerName: string; roomId: string }, callback: (response: any) => void) => {
        this.handleJoinRoom(socket, data, callback);
      });

      // 创建房间事件
      socket.on('create_room', (data: { hostId: string; roomName: string }, callback: (response: any) => void) => {
        this.handleCreateRoom(socket, data, callback);
      });

      // 获取可用房间列表
      socket.on('list_rooms', (callback: (response: any) => void) => {
        this.handleListRooms(callback);
      });

      // 开始游戏事件
      socket.on('start_game', (data: { roomId: string; hostId: string }, callback: (response: any) => void) => {
        this.handleStartGame(socket, data, callback);
      });

      // 游戏决策事件
      socket.on('make_decision', (data: any, callback: (response: any) => void) => {
        this.handleMakeDecision(socket, data, callback);
      });

      // 掷骰子事件
      socket.on('roll_dice', (data: any, callback: (response: any) => void) => {
        this.handleRollDice(socket, data, callback);
      });

      // 年度结算事件
      socket.on('annual_settle', (data: any, callback: (response: any) => void) => {
        this.handleAnnualSettle(socket, data, callback);
      });

      // 消息事件（用于玩家间通信）
      socket.on('send_message', (data: { message: string }, callback: (response: any) => void) => {
        this.handleSendMessage(socket, data, callback);
      });

      // 离开房间事件
      socket.on('leave_room', (data: { playerId: string }, callback: (response: any) => void) => {
   ...
   * 处理获取房间列表
   */
  private handleListRooms(callback: (response: any) => void): void {
   ...
   * 处理游戏决策
   */
  private handleMakeDecision(socket: Socket, data: any, callback: (response: any) => void): void {
   ...
   * 处理掷骰子
   */
  private handleRollDice(socket: Socket, data: any, callback: (response: any) => void): void {
    const playerId = socket.data?.playerId;
    const roomId = socket.data?.roomId;
   ...
   * 处理年度结算
   */
  private handleAnnualSettle(socket: Socket, data: any, callback: (response: any) => void): void {
    const playerId = socket.data?.playerId;
    const roomId = socket.data?.roomId;
   ...
   * 处理发送消息
   */
  private handleSendMessage(socket: Socket, data: { message: string }, callback: (response: any) => void): void {
    const playerId = socket.data?.playerId;
   ...
   * 处理离开房间
   */
  private handleLeaveRoom(socket: Socket, data: { playerId: string }, callback: (response: any) => void): void {
    const { playerId } = data;
    const roomId = socket.data?.roomId;
