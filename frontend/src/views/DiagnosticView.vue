      ...
      <div class="diagnostic-section">
        ...
        <div class="check-item" :class="backendStatus">
          ...
          <span class="status-text">
            后端服务: 
            {{ backendStatus === 'checking' ? '检测中...' : (backendStatus === 'ok' ? '✅ 正常运行' : '❌ 无法连接') }}
          </span>
...
async function checkBackend() {
  ...
  try {
    ...
    console.log('⏳ 检查后端服务...', backendBase);
    ...
    if (response.ok) {
      // 获取格子定义
      const gridResponse = await fetch(`${apiBase}/grid/definitions`, {
        signal: controller.signal
      });
    ...
    if (err.name === 'AbortError') {
      ...
      backendError.value = '❌ 网络错误：无法连接到后端服务器\n请检查VITE_API_BASE环境变量配置';
    } else {
