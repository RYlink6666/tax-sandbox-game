...
onMounted(async () => {
  try {
    console.log('🔌 正在连接到后端: https://tax-sandbox-game.onrender.com/api/grid/definitions');
    
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 5000);
    
    const response = await fetch('https://tax-sandbox-game.onrender.com/api/grid/definitions', {
